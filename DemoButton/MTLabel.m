//
//    Modification of MTLabel to support setting of state and shadow attributes
//
...
@synthesize selectedShadowColor;
...
- (void)drawTextInRect:(CGRect)rect inContext:(CGContextRef)context {
    ...
    if (selected) {
        attributes = [NSDictionary dictionaryWithObjectsAndKeys:
                                    ...
                                    selectedFontColor.CGColor, kCTForegroundColorAttributeName,
                                    nil];
    } else {
        attributes = [NSDictionary dictionaryWithObjectsAndKeys:
                                    (id)font, (id)kCTFontAttributeName,
...
- (void)drawRect:(CGRect)rect {
    ...
    CGColorRef colorRef;
    if (selected) {
        colorRef = CGColorCreate(CGColorSpaceCreateDeviceRGB(), CGColorGetComponents(selectedFontColor.CGColor));
        CGContextSetShadowWithColor(context, shadowOffset, 0, selectedShadowColor.CGColor);
    } else {
        colorRef = CGColorCreate(CGColorSpaceCreateDeviceRGB(), CGColorGetComponents(_fontColor.CGColor));
